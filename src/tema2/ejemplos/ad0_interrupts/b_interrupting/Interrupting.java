...
//        firstExample();
    ...
    /**
     * <p>
     * start a thread, interrupt it and (try to) join to it.
     * <p>
     * The interruption is probably received by task's thread when has yet
     * execute 'sleep()', so sleep() throws InterruptedException, that is couch
     * in 'run()' method that simply -show a message and- returns.
     * <p>
     * note: <code>run()</code> CAN'T throws <code>InterruptedException</code>,
     * �so <code>join()</code> can't throw this exception?
     */
    ...
    /**
     * <p>
     * If we execute this method several times, before invoking
     * <code>cancel()</code>, will happend:
     *
     * <ul>
     * <li><p>
     * if task's thread has been scheduled yet, the <code>sleep()</code> method
     * in the task code, will throw an <code>ExecutionException</code>, and in
     * main's thread <code>get()</code> method will throw an
     * <code>CancelationException</code>
     * <li><p>
     * if task's thread hasn't been scheduled yet, only in main's thread
     * <code>get()</code> method will throw an <code>CancelationException</code>
     * </ul>
     *
     * <p>
     * If <code>shutdown()</code> is placed <i>after</i> <code>cancel()</code>,
     * only <code>CancelationException</code> will be thrown, because
     * <code>shutdown()</code> waits until the scheduled task finish.
     * <p>
     * <code>shutdownNow()</code> will try to cancel inmediatly current
     * scheduled tasks, so some times the both <code>ExecutionException</code>
     * and <code>CancelationException</code> can't be thrown
     *
     *
     */
